...
public class InboxPage extends BasePages {
	...
	@AndroidFindBy(xpath = "//*[@resource-id='com.answerconnect.mobile:id/swipe_reveal_layout'][3]/*[@resource-id='com.answerconnect.mobile:id/container']/child::android.widget.TextView[1]")
	private WebElement inboxMessageThreeName;

	@AndroidFindBy(xpath = "//*[@resource-id='com.answerconnect.mobile:id/swipe_reveal_layout'][3]/*[@resource-id='com.answerconnect.mobile:id/container']/child::android.widget.TextView[2]")
	private WebElement inboxMessageThreeDate;

	@AndroidFindBy(xpath = "//*[@resource-id='com.answerconnect.mobile:id/swipe_reveal_layout'][4]/*[@resource-id='com.answerconnect.mobile:id/container']/child::android.widget.TextView[1]")
	private WebElement inboxMessageFourName;

	@AndroidFindBy(xpath = "//*[@resource-id='com.answerconnect.mobile:id/swipe_reveal_layout'][4]/*[@resource-id='com.answerconnect.mobile:id/container']/child::android.widget.TextView[2]")
	private WebElement inboxMessageFourDate;
	...
	@AndroidFindBy(id = "//*[@resource-id='com.answerconnect.mobile:id/swipe_reveal_layout'][3]")
	...
	private WebElement moveToArchiveOrInbox;
...
//		swipeUp();
		...
		String dateT = inboxMessageThreeDate.getText().trim();
		System.out.println(nameT + "   " + dateT);
		try {
			longPress("Inbox message", inboxMessageThreeName);
			tapOn("Archive", archive);
			tapOn("More option", moreOptionsInbox);
			tapOn("Archive toggle", archiveFilter);
			tempWait(2000);
			scrollUntil(dateT);
			scrollInto(nameT);
			getBase64();
			test.log(LogStatus.PASS, "Inbox to Archive successful");
		} catch (Exception e) {
			e.printStackTrace();
			test.log(LogStatus.FAIL, "Inbox to Archive failed or not enough message");
		}

		// Archive to Inbox outside
		try {
		...
		String nameF = inboxMessageFourName.getText().trim();
		String dateF = inboxMessageFourDate.getText().trim();
		try {
			System.out.println(nameF + "   " + dateF);
			longPress("Inbox message", inboxMessageFourName);
			tapOn("Trash", trash);
			tapOn("More option", moreOptionsInbox);
			...
			scrollInto(nameF);
			getBase64();
			test.log(LogStatus.PASS, "Inbox to Trash successful");
		} catch (Exception e) {
			e.printStackTrace();
			test.log(LogStatus.FAIL, "Inbox to Trash failed or not enough message");
		}

		// Trash to Inbox outside
		try {
			tapOn("options", messageMoreOption);
			tapOn("Back to inbox", moveToArchiveOrInbox);
		...
		try {
			if (headerForConfirmation.isDisplayed()) {
				...
				// Chat Icon Inside
				try {
					tapOn("more options", contactMoreOptions);
					tapOn("Edit profile", editContact);
					String temp = contactName.getText().trim();
					goBack();
					goBack();
		...
		try {
			...
			tapOn("move to archive", moveToArchiveOrInbox);
