...
public class InboxPage extends BasePages {
	...
	public void inbox() throws InterruptedException {
		...
		if (allIconChecked.isDisplayed()) {
			try {
				...
				if (inboxNoMessageToast.isDisplayed()) {
					tests.log(LogStatus.PASS, "No messages for Unread");
				} else if (!inboxNoMessageToast.isDisplayed()) {
					tapOn("Message icon", inboxMessageIcon);
					if (markAsRead.isDisplayed()) {
						tests.log(LogStatus.PASS, "All dropdown unread is passed");
						tapOn("Message icon", inboxMessageIcon);
					}
				}
			} catch (Exception e) {
				tests.log(LogStatus.FAIL, "All dropdown unread is failed");
		...
		try {
			...
			tests.log(LogStatus.PASS, "Archive success");
		} catch (Exception e) {
			System.out.println(e.getMessage());
			tests.log(LogStatus.FAIL, "Archive failed");
		}

		// Validating a message view
		try {
			String texts = inboxMessage.getText();
			...
			tapOn("Go back", navigateBack);
			System.out.println(e.getMessage());
			tests.log(LogStatus.FAIL, "Message validation failed");
		}
		...
		try {
			if (headerForConfirmation.isDisplayed()) {
				...
				try {
					...
					tests.log(LogStatus.FAIL, "Chat icon validation unsuccessful");
				}
				...
				try {
					...
					tests.log(LogStatus.FAIL, "Phone number validation unsuccessful");
				...
				try {
					...
					tests.log(LogStatus.FAIL, "Bottomsheet phone number validation unsuccessful");
				...
				try {
					...
					tests.log(LogStatus.FAIL, "Bottomsheet email validation unsuccessful");
				...
				try {
					...
					System.out.println(e.getMessage());
					tests.log(LogStatus.FAIL, "Bottomsheet phone copy validation unsuccessful");
					goBack();
				}
				...
				try {
					...
					System.out.println(e.getMessage());
					tests.log(LogStatus.FAIL, "Bottomsheet email copy validation unsuccessful");
					goBack();
				}
				tapOn("Go back", profileBackButton);
			...
			System.out.println(e.getMessage());
			tests.log(LogStatus.FAIL, "Unsaved contact");
		}
