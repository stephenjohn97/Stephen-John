			...
			if (businessLineView.isDisplayed()) {
				...
				if (businessLineDropdown.isDisplayed()) {
					...
					if (businessLineBottomsheet.isDisplayed()) {
						...
						tests.log(LogStatus.PASS, "Bottomsheet success");
					} else {
						tests.log(LogStatus.FAIL, "Bottomsheet failed");
					}
				} else {
					tests.log(LogStatus.FAIL, "Dropdown unavailable");
				}
				...
				try {
					...
					tests.log(LogStatus.FAIL, "Create New Contact failed");
				}

				try {
					tapOn("New SMS", newSMSCalls);
					if (newSMSToNumField.isDisplayed()) {
						tests.log(LogStatus.PASS, "New SMS view validated");
						...
						tests.log(LogStatus.FAIL, "New SMS view failed");
					}

				} catch (Exception e) {
					tests.log(LogStatus.FAIL, "New SMS failed");
				}

				try {
					tapOn("My Team", myTeam);
					if (myTeam.isSelected()) {
						tests.log(LogStatus.PASS, "'My Team' is selected");
					} else {
						tests.log(LogStatus.FAIL, "My Team is not selected");
					}
					tapOn("Contacts", contacts);
					if (contacts.isSelected()) {
						tests.log(LogStatus.PASS, "'Contacts' is selected");
					} else {
						tests.log(LogStatus.FAIL, "'Contact' is not selected");
					}
				} catch (Exception e) {
					tests.log(LogStatus.FAIL, "My Team and Contacts selection failed");
				}
				...
				try {
					...
					tests.log(LogStatus.FAIL, "New Contact failed");
				}

			} else {
				tests.log(LogStatus.INFO, "Business line doesn't exist");
			}

		} catch (Exception e) {
			tests.log(LogStatus.INFO, "BUSINESS LINE UNAVAILABLE FOR: " + prop.getProperty("emailId"));
		}
	}
