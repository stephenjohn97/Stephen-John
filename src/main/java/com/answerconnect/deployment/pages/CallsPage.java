...
public class CallsPage extends BasePages {
	...
	@AndroidFindBy(id = "com.answerconnect.mobile:id/request_early_access_tv")
	private WebElement requestEarlyAccess;

	public void calls() throws InterruptedException {
		tapOn("Calls tab", moreTab);
		try {
			...
			tapOn("calls tab", callsTab);
			if (requestEarlyAccess.isDisplayed()) {
				test.log(LogStatus.INFO, "BUSINESS LINE IS UNAVAILABLE FOR: " + prop.getProperty("emailId"));
			} else {
				e.printStackTrace();
				test.log(LogStatus.FAIL, "Something went wrong in Calls tab");
			}
		}
	}
